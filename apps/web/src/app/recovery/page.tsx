...
export default function RecoveryPage() {
  ...
  const [info, setInfo] = useState<{ species: number; schemaVersion: number; updatedAt: string } | null>(null);
  ...
  useEffect(() => {
    try {
      ...
      if (data) {
        ...
        setInfo({
          species: Array.isArray(parsed.collection) ? parsed.collection.length : 0,
          schemaVersion: parsed.schemaVersion ?? 0,
          updatedAt: parsed.updatedAt ? new Date(parsed.updatedAt).toLocaleString() : 'unknown',
        });
        // Build export string (florify1: + base64(gzip(json)))
        buildExportString(parsed).then(setExportStr).catch(() => {
          // Fallback: just base64 the raw JSON without gzip
          setExportStr('florify-raw:' + btoa(unescape(encodeURIComponent(data))));
        });
      }
    } catch (e) {
      setError(`Failed to read localStorage: ${e instanceof Error ? e.message : 'unknown'}`);
    }
  }, []);
  ...
  return (
    <div style={{ padding: 24, fontFamily: 'system-ui, sans-serif', maxWidth: 480, margin: '0 auto' }}>
      ...
      <p style={{ fontSize: 12, color: '#888', marginBottom: 16 }}>
        Build: <strong>{buildVersion}</strong> · Schema: <strong>v{SCHEMA_VERSION}</strong>
      </p>
      ...
      {info && (
        <div style={{ background: '#f0f7f0', padding: 12, borderRadius: 8, marginBottom: 16 }}>
          <div>Species collected: <strong>{info.species}</strong></div>
          <div>Save schema: <strong>v{info.schemaVersion}</strong></div>
          <div>Last updated: <strong>{info.updatedAt}</strong></div>
        </div>
      )}
      ...
      {raw && (
        <details style={{ marginTop: 24 }}>
          ...
          <button
            ...
            style={{ marginTop: 8, padding: '8px 12px', fontSize: 13, borderRadius: 8, border: '1px solid #ddd', background: '#fafafa', cursor: 'pointer' }}
          >
            Copy Raw JSON
          </button>
          <pre style={{ marginTop: 8, fontSize: 11, background: '#f5f5f5', padding: 12, borderRadius: 8, overflow: 'auto', maxHeight: 300 }}>
      ...
      <div style={{ marginTop: 32, textAlign: 'center' }}>
        <a href="/" style={{ color: '#2563eb', fontSize: 14 }}>Back to Florify</a>
      </div>
    </div>
  );
}
...
async function buildExportString(state: Record<string, unknown>): Promise<string> {
  const envelope = {
    format: 'florify-save',
    formatVersion: 1,
    exportedAt: Date.now(),
