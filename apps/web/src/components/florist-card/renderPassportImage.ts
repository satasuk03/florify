...
export async function renderPassportImage(
  ...
  // Wait for custom fonts (Fraunces, IBM Plex Sans Thai Looped) so
  // canvas doesn't fall back to generic serif/sans mid-render.
  try {
    await document.fonts.ready;
  } catch {
    // Some environments lack FontFaceSet; we'll fall back to defaults.
  }
...
function drawImageOp(
  ...
  // Background fill first (always drawn, even for placeholder).
  ctx.save();
  ctx.beginPath();
  if (r > 0) pathRoundedRect(ctx, op.x, op.y, op.w, op.h, r);
  else ctx.rect(op.x, op.y, op.w, op.h);
  ctx.closePath();
  ctx.fillStyle = op.bgColor;
  ctx.fill();

  // Clip and draw — object-fit: cover.
  ctx.clip();
...
function drawText(ctx: CanvasRenderingContext2D, op: Extract<DrawOp, { type: 'text' }>): void {
  ...
  ctx.font = `${style}${op.weight} ${op.size}px ${canvasFontFamily(op.family)}`;
  ctx.fillStyle = op.color;
  applyTextShadow(ctx, op.shadow);
  ...
  // Fallback: measure + draw per-char.
  const chars = [...op.text];
...
function drawGradientRect(
  ctx: CanvasRenderingContext2D,
  op: Extract<DrawOp, { type: 'gradientRect' }>,
): void {
  // Match CSS linear-gradient(angle, ...): angle 0° = bottom→top,
  // 180° = top→bottom. Convert into a vector across the rect.
  const angleRad = (op.angle - 90) * (Math.PI / 180);
  const cx = op.x + op.w / 2;
  const cy = op.y + op.h / 2;
  // Project half-diagonal onto the gradient direction so start/end
  // sit at the bounding box's edges along that axis.
  const halfLen =
    (Math.abs(Math.cos(angleRad)) * op.w + Math.abs(Math.sin(angleRad)) * op.h) / 2;
...
function canvasFontFamily(family: 'serif' | 'sans' | 'mono'): string {
  // Canvas 2D can't read CSS variables — use explicit names matching
  // what next/font loads under.
  switch (family) {
    case 'serif':
